...
function openBaicOperationModal(leadId) {
  ...
  $("baicOperationSummary").innerHTML = `<div class="baic-workbench-task"><span>我的当前任务 · ${esc(lead.taskStatus)}</span><strong>${esc(lead.task)}</strong><small>任务ID：${esc(lead.taskId)} · 截止时间：${esc(lead.taskDueAt)}</small></div><div class="baic-workbench-user"><div><span>线索ID</span><strong>${esc(lead.id)}</strong></div><div><span>姓名</span><strong>${esc(lead.name)}</strong></div><div><span>手机号</span><strong>${esc(lead.phone)}</strong></div><div><span>线索类型</span><strong>${esc(lead.type)}</strong></div><div><span>车系</span><strong>${esc(lead.series)}</strong></div><div><span>车型</span><strong>${esc(lead.model)}</strong></div><div><span>线索来源</span><strong>${esc(lead.source)}</strong></div><div><span>当前线索状态</span><strong>${esc(lead.status)}</strong></div></div>`;
