...
function openBaicOperationModal(leadId) {
  ...
  $("baicOperationTitle").textContent = "线索跟进工作台";
  $("baicOperationSummary").innerHTML = `<div class="baic-workbench-task"><span>我的当前任务 · ${esc(lead.taskStatus)}</span><strong>${esc(lead.task)}</strong><small>触发原因：北汽线索待处理</small><code>任务ID：${esc(lead.taskId)} · 截止时间：${esc(lead.taskDueAt)}</code></div><section class="baic-workbench-card"><div class="baic-workbench-card-title"><h3>用户信息</h3><span>线索ID：${esc(lead.id)}</span></div><div class="baic-workbench-user"><div><span>姓名</span><strong>${esc(lead.name)}</strong></div><div><span>手机号</span><strong>${esc(lead.phone)}</strong></div><div><span>线索类型</span><strong>${esc(lead.type)}</strong></div><div><span>当前线索状态</span><strong>${esc(lead.status)}</strong></div><div><span>车系</span><strong>${esc(lead.series)}</strong></div><div><span>车型</span><strong>${esc(lead.model)}</strong></div><div><span>线索来源</span><strong>${esc(lead.source)}</strong></div><div><span>城市</span><strong>${esc(lead.city || "—")}</strong></div></div></section><section class="baic-workbench-card"><div class="baic-workbench-card-title"><h3>本次跟进</h3><span>当前任务决定跟进内容</span></div></section><section class="baic-workbench-activity"><div class="baic-workbench-tabs"><strong>操作记录</strong><span>跟踪记事</span></div><p>系统自动记录本次线索处理，提交后将在列表中保留任务完成记录。</p></section>`;
