...
$('#leadStateSelect').addEventListener('change',e=>renderLeadState(e.target.value)); $('#leadStateSelect').value='following'; renderLeadState('following');
...
function renderLeadActivity(lead){$('#currentName').textContent=lead.name;$('#currentPhone').textContent=lead.phone;$('#currentBrand').textContent=lead.brand;$('#currentSeries').textContent=lead.series;$('#currentModel').textContent=lead.model;$('#opsTimeline').innerHTML=`<div class="timeline-item"><span class="timeline-dot gray"></span><div><div class="timeline-head"><b>进入当前工作台</b><time>刚刚</time></div><p>系统根据销售提交结果切换到下一条线索</p><div class="operator-line">操作人：系统</div></div></div><div class="timeline-item"><span class="timeline-dot blue"></span><div><div class="timeline-head"><b>线索已创建</b><time>今天 09:30</time></div><p>来源：系统自动 · 线索中心</p><div class="operator-line">操作人：系统</div></div></div>`;$('#noteTimeline').innerHTML=lead.notes.map((n,i)=>`<div class="timeline-item"><span class="timeline-dot purple"></span><div><div class="timeline-head"><time>${i?'昨天 16:40':'今天 10:18'}</time></div><p>${escapeHtml(n)}</p><div class="operator-line">操作人：sales 001 Deng Yao</div></div></div>`).join('');$('#noteCount').textContent=lead.notes.length;}
function loadNextLead(){currentLeadIndex=(currentLeadIndex+1)%leadQueue.length;const lead=leadQueue[currentLeadIndex];$('#leadStateSelect').value=lead.state;renderLeadState(lead.state);$('#currentMainState').textContent=lead.main;$('#currentSubState').textContent=lead.sub;renderLeadActivity(lead);showToast(`已提交，进入下一条：${lead.name}`);}
$('#completeBtn').addEventListener('click', () => { if(!$('#lostReasonWrap').classList.contains('hidden') && !$('#lostReason').value){ showToast('请先选择战败原因'); $('#lostReason').focus(); return; } loadNextLead(); });
let allLeadRows=[];
...
fetch('./data.json').then(response=>{if(!response.ok)throw new Error('data.json load failed');return response.json();}).then(data=>{leadQueue=data.leads;allLeadRows=data.leadRows;renderLeadRows();renderLeadState('following');renderLeadActivity(leadQueue[0]);}).catch(()=>showToast('演示数据加载失败，请确认 data.json 已上传'));
